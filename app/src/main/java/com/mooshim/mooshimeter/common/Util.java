...
public class Util {
    ...
    public static void init(Context context) {
        ...
        TextToSpeech.OnInitListener speaker_init_listener = i -> {
    ...
    public static FirmwareFile downloadLatestFirmware() {
        if (download_fw == null) {
            ...
            if (tmp != null) {
                ...
                Timber.d("Failed to download newer firmware.");
                download_fw = null;
            }
        }
    ...
    public static void checkNotOnMainThread() {
        if (inMainThread()) {
            Timber.e("We're in the main thread, but we should not be!");
    ...
    public static int offerChoiceDialog(final Context context, final String title, final String message, final String[] buttons) {
        ...
        if (buttons.length > 3) {
            Timber.e("Can't have more than 3 choices!");
    ...
    public static PopupMenu generatePopupMenuWithOptions(final Context context, final List<String> options, final View anchor, final NotifyHandler cb, final Runnable on_dismiss) {
        ...
        for (String s : options) {
            ...
            rval.setOnMenuItemClickListener(menuItem -> {
                final int choice = menuItem.getItemId();
                rval.dismiss();
                Util.dispatch(() -> cb.onReceived(getUTCTime(), choice));
                return false;
            });
            i++;
        }
        if (on_dismiss != null) {
            rval.setOnDismissListener(popupMenu -> on_dismiss.run());
        }
        rval.show();
        return rval;
    }
